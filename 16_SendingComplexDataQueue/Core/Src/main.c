...
#include <string.h>
#include "main.h"
#include "cmsis_os.h"  // <-- include<queue.h>
...
int main(void)
{
  ...
  SimpleQueue = xQueueCreate(3, sizeof (Data_t));
    ...
    // Sender task1 & task2 same priority
    //lower than the receive task priority
    ...
    // Create a Receiver task high priority
...
void SenderTask(void *pvParameters)
{
	...
	while (1)
	{
		if (xQueueSend(SimpleQueue, pvParameters, portMAX_DELAY) == pdPASS)
		{
			printf(" Successfully sent1 the number to the queue\r\n");
		}
...
void ReceiverTask(void *pvParameters)
{
	...
	while (1)
	{
		if (xQueueReceive(SimpleQueue, &xReceivedStructure, portMAX_DELAY) != pdTRUE)
		{
		...
		else
		{
			...
			{
				printf(" Successfully RECEIVED Humidity Sensor value %d \r\n", xReceivedStructure.ucValue);
			}
			else
			{
				printf(" Successfully RECEIVED Pressure Sensor value %d \r\n", xReceivedStructure.ucValue);
			}
		}
		vTaskDelay(TickDelay);
	}
