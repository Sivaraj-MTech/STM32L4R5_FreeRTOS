...
void SenderTask2(void *pvParameters)
{
	...
	while (1)
	{
		if (xQueueSend(SimpleQueue, &i, portMAX_DELAY) == pdPASS)
		{
			printf(" Successfully sent2 the number to the queue\r\n");
		}
		vTaskDelay(TickDelay);
	}
}
...
void ReceiverTask(void *pvParameters)
{
	...
	while (1)
	{
		...
		{
			printf(" Successfully RECEIVED the number to the queue %ld\r\n", received);
		}
		vTaskDelay(TickDelay);
	}
}
...
PUTCHAR_PROTOTYPE
{
  // Example for UART3, assuming huart3 is initialized
	uart3_write(ch);
	return ch;
}
